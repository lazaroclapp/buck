...
/**
 * Used to expand the macro {@literal $(processorpath //some:target)} to the transitive
 * processorpath of that target, expanding all paths to be absolute.
 */
public class ProcessorpathMacroExpander extends BuildTargetMacroExpander<ProcessorpathMacro> {

  @Override
  public Class<ProcessorpathMacro> getInputClass() {
    return ProcessorpathMacro.class;
  }

  @Override
  protected ProcessorpathMacro parse(
      BuildTarget target, CellPathResolver cellNames, ImmutableList<String> input)
      throws MacroException {
    return ProcessorpathMacro.of(parseBuildTarget(target, cellNames, input));
  }
  ...
  private HasClasspathEntries getHasClasspathEntries(BuildRule rule) throws MacroException {
    if (!(rule instanceof HasClasspathEntries)) {
      throw new MacroException(
          String.format(
              "%s used in processorpath macro does not correspond to a rule with a java " +
              "processorpath",
              rule.getBuildTarget()));
    }
